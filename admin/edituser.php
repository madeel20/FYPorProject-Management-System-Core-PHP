...
// if everything is ok, try to upload file
    ...
    $sql ="select username from tbl_student where id=".$_POST['userid'];
...
$uss = $row['username'];
    ...
    $imgsrc = $target_dir.$_POST['userid'].$uss.'.'.$imageFileType;
		if (move_uploaded_file($_FILES["img"]["tmp_name"], $imgsrc)) {
			$sql = "UPDATE `tbl_student` SET `img`='".$imgsrc."' WHERE id=".$_POST['userid'];
           if($con->query($sql)){
...
include 'connection.php';
	...
	$email = $_POST['em'];
	$gender = $_POST['gender'];
        $sql = "UPDATE `tbl_student` set `fname`='".$fname."',`lname`='".$lname."',`university`='".$university."',`country`='".$country."',`username`='".$username."',`pass`='".$pass."',`email`='".$email."',`gender`='".$gender."',`qualifications`='".$qual."' WHERE id=".$_POST['userid'];
...
<!--  page banner -->
 ...
 <div class="row">
   ...
   <h2 class="page-banner-heading  text-center">Admin Panel:  </h2><p>Edit User:</p>
   </div>
   <div class="col-sm-2"></div>
</div>
  </section> 
