...
// if everything is ok, try to upload file
    ...
    $sql ="select username from tbl_supervisor where id=".$_SESSION['id'];
...
$uss = $row['username'];
    ...
    $imgsrc = $target_dir.$_SESSION['id'].$uss.'.'.$imageFileType;
		if (move_uploaded_file($_FILES["img"]["tmp_name"], $imgsrc)) {
			$sql = "UPDATE `tbl_supervisor` SET `img`='".$imgsrc."' WHERE id=".$_SESSION['id'];
           if($con->query($sql)){
               $_SESSION['img']=$imgsrc;
...
include 'connection.php';
	...
	$email = $_POST['em'];
	$gender = $_POST['gender'];
        $sql = "UPDATE `tbl_supervisor` set `fname`='".$fname."',`lname`='".$lname."',`university`='".$university."',`country`='".$country."',`username`='".$username."',`pass`='".$pass."',`email`='".$email."',`gender`='".$gender."',`qualifications`='".$qual."' WHERE id=".$_SESSION['id'];
...
<!--  page banner -->
 ...
 <div class="row">
   ...
   <h2 class="page-banner-heading  text-center">Edit Details: </h2>
   </div>
   <div class="col-sm-2"></div>
</div>
  </section> 
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
     ...
     echo ''.$row['username'].'</h2></td><td><form enctype="multipart/form-data"  method="post"  action="#">      <input type="hidden" name="userid" value="'.$_SESSION['id'].'"/>
