...
// if everything is ok, try to upload file
    ...
    $sql ="select username from tbl_student where id=".$_SESSION['id'];
...
$uss = $row['username'];
    ...
    $imgsrc = $target_dir.$_SESSION['id'].$uss.'.'.$imageFileType;
		if (move_uploaded_file($_FILES["img"]["tmp_name"], $imgsrc)) {
			$sql = "UPDATE `tbl_student` SET `img`='".$imgsrc."' WHERE id=".$_SESSION['id'];
           if($con->query($sql)){
               $_SESSION['img']=$imgsrc;
...
include 'connection.php';
	...
	$email = $_POST['em'];
	$gender = $_POST['gender'];
        $sql = "UPDATE `tbl_student` set `fname`='".$fname."',`lname`='".$lname."',`university`='".$university."',`country`='".$country."',`username`='".$username."',`pass`='".$pass."',`email`='".$email."',`gender`='".$gender."',`qualifications`='".$qual."' WHERE id=".$_SESSION['id'];
